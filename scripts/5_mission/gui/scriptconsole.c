...
class ScriptConsole extends UIScriptedMenu
{
	...
	override Widget Init()
	{
		...
		m_LocationAddButton	= ButtonWidget.Cast(layoutRoot.FindAnyWidget("AddButton"));
		m_LocationRemoveButton	= ButtonWidget.Cast(layoutRoot.FindAnyWidget("RemoveButton"));
		m_FilterOrderButton	= ButtonWidget.Cast(layoutRoot.FindAnyWidget("ObjectFilterSortButton"));
		m_DumpParamButton	= ButtonWidget.Cast(layoutRoot.FindAnyWidget("DumpParamButton"));
		m_FilterOrderImage	= ImageWidget.Cast(layoutRoot.FindAnyWidget("ObjectFilterSortPic"));
		...
		m_EnfScriptClear	= ButtonWidget.Cast(layoutRoot.FindAnyWidget("ClearButton"));
...
/*
	...
	*/
	...
	void RunEnscript()
	{
		...
		code.Replace("Print(","ScriptConsole.PrintS(");
		code.Replace("Print (","ScriptConsole.PrintS(");
	...
	*/
	...
	override bool OnClick(Widget w, int x, int y, int button)
	{
		...
		{
			ScriptConsoleAddLocation menu = ScriptConsoleAddLocation.Cast(g_Game.GetUIManager().EnterScriptedMenu(MENU_LOC_ADD, this));
	...
	void SetPreviewObject(string object)
	{
		...
		if (!GetGame().IsKindOf(object, "DZ_LightAI") && !GetGame().IsKindOf(object, "Man"))
		{
			...
			m_PreviewEntity = EntityAI.Cast(GetGame().CreateObjectEx(object, "0 0 0", ECE_LOCAL | ECE_PLACE_ON_SURFACE));
...
//		m_ItemQuantityLabel.Show(false);
	...
	void ReloadOutput()
	{
		...
		{
			string line_content;
			while ( FGets( file_index,  line_content ) > 0 )
			{
