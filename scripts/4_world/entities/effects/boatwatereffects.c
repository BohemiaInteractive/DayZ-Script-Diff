...
class SoundBoatWaterBack : EffectSound
{
	protected const int SPEED_FAST = 40;
	protected const float SPLASH_THRESHOLD_CONDITION = 0.1; // diff between sea surface and propeller pos which determines if the splash should happen
	...
	void Update()
	{
		float propPosRelative = GetGame().SurfaceGetSeaLevel() - m_Boat.CoordToParent(m_Boat.PropellerGetPosition())[1];
		...
		if (m_SplashIncoming && propPosRelative > SPLASH_THRESHOLD)
		{
			vector velocity = dBodyGetVelocityAt(m_Boat, m_Boat.CoordToParent(GetLocalPosition()));
			float speed = velocity.Normalize() * 3.6; // to km/h
			float lerp = Math.InverseLerp(0, SPEED_FAST, speed);
			SetSoundVolume(lerp);
			
			SetCurrentPosition(m_Boat.CoordToParent(m_MemPointPos));
